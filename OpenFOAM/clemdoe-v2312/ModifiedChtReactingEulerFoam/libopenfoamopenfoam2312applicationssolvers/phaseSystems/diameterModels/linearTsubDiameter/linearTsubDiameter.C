...
:
    // Este seteo lo habia tocado yo. Pero al final lo deje como estaba oroginal por OpenFOAM

    /*diameterModel(diameterProperties, phase),
    liquidPhaseName_(diameterProperties.lookup("liquidPhase")),
    d2_("d2", dimLength, diameterProperties.lookupOrDefault("d2", 0.0021)), //0.0015)),
    ...
    (
        "Tsub2",
         dimTemperature,
        // diameterProperties.lookupOrDefault("Tsub2", -5.0)  //Ojo!! Esto antes tenia como valor 0
        diameterProperties.lookupOrDefault("Tsub2", 13.5)  //Ojo!! Esto antes tenia como valor 0
    ),
    d1_
    (
        "d1",
        dimLength,
        diameterProperties.lookupOrDefault("d1", 0.00045) //0.00015)
    ),
    Tsub1_
    (
        "Tsub1",
       ...
       // diameterProperties.lookupOrDefault("Tsub1", 13.5)
        diameterProperties.lookupOrDefault("Tsub1", -5.0)
    ),
    d_
    (
        IOobject
    ...
    // Version Original de OpenFOAM

    diameterModel(diameterProperties, phase),
      liquidPhaseName_(diameterProperties.lookup("liquidPhase")),
      d2_("d2", dimLength, diameterProperties.lookupOrDefault("d2", 0.001)),
      Tsub2_
      (
          "Tsub2",
           dimTemperature,
      ...
      (
          "d1",
          dimLength,
          diameterProperties.lookupOrDefault("d1", 0.000015)
      ),
      Tsub1_
      (
          "Tsub1",
...
void Foam::diameterModels::linearTsub::correct()
{
    ...
    if (phase_.mesh().foundObject<saturationModel>("saturationModel"))
    {
        ...
        //volScalarField Tliquido (liquid.thermo().T());
        //volScalarField Tsatura (satModel.Tsat(liquid.thermo().p()));

        //Foam::Info << "Temperatura del liquido: " << Tliquido.primitiveField() << endl;
        //Foam::Info << "Temperatura del liquido: " << Tsatura.primitiveField() << endl;

        /*Foam::Info << "El diametro d1 es:   " <<  d1_.value() << endl;
        Foam::Info << "El diametro d2 es:   " <<  d2_.value() << endl;
        Foam::Info << "La Tsub1 es:   " <<  Tsub1_.value() << endl;
        Foam::Info << "La Tsub2 es:   " <<  Tsub2_.value() << endl;*/
