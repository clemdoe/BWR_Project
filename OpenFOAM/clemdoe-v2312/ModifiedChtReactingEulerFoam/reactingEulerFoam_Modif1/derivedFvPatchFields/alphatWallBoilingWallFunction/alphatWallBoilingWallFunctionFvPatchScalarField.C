...
void alphatWallBoilingWallFunctionFvPatchScalarField::updateCoeffs()
{
    ...
    switch (phaseType_)
    {
        ...
        //Info << "Entra a la Fase vapor " << endl;
            ...
            const fvPatchScalarField& hev
                = vapor.thermo().he().boundaryField()[patchi];

            //El hev es la entalpia de vapor
            //Info << "Esto es: " << hev << endl;

            const tmp<scalarField> talphav = vapor.thermo().alpha(patchi);
            const scalarField& alphav = talphav();

            //El alphav es aca la difusividad termica del material
            //Basicametne el calculo que hace es mu/(Pr rho). Pero el rho no lo tiene en cuenta
            // porque lo debe tener en cuenta en otro momento
            //Info << "Esto ES: " << alphav << endl;
            ...
            //Pude verificar que cuando es todo liquido el dominio
            // aca fliquid da todo casi 1, por lo que mas abajos
            // cuando hacer 1 - fliquid va a dar casi cero
            //Info << "Funcion fliquid: " << (1-fLiquid) << endl;

            //Info << "Reporto *this" << *this << endl;

            // El calcAlphat(*this) lo que hace es calcular el Alphat que seria
            // la difusividad turbulenta


            // El snGrad() calcula el gradiente de la variable hev, donde hev es la entalpia de vapor
            // Entonces retorna el gradietne de la entalpia
            //Info << "hev.snGrad: " << hev.snGrad() << endl;

            const scalarField qconvv
            (
                (calcAlphat(*this)+alphav)*(1 - fLiquid)*hev.snGrad()
            );

//            Info << "Reporto los flujos de calor en vapor"<<endl;
//            Info << "z[m]" <<"\t" << "qconvVap" << " \t \t" << "fLiquid-1"  << endl;
//            forAll(qconvv,patchi)
//            {
//             Info << patch().Cf()[patchi][2] << "\t"
//                  << qconvv[patchi] << "\t"
//                  << 1-fLiquid[patchi] << endl;
//            }

            Info << "Entre al operator== vapor" << endl;
            operator==
            (
                calcAlphat(*this)*(1 - fLiquid)/max(vaporw, scalar(1e-8))
                //0*(calcAlphat(*this)*(1 - fLiquid)/max(vaporw, scalar(1e-8)))
            );
            break;
        }
        case liquidPhase:
        {
           ...
           // Foam::Info << "El valor de Tc es: " << Tc << endl;
            ...
            //Info << "oldAlphatConv" << alphatConv_ <<endl;
            alphatConv_ = calcAlphat(alphatConv_);
            //Info << " newAlphatConv" << alphatConv_ << endl;


            //Info << "Reporto fLiquid" << fLiquid << endl;
            ...
            for (label i=0; i<10; i++)
            {
                ...
                (
                    //Esto es una prueba que estoy haciendo
...
//                   //const scalarField A((TsupUnal/(2.0*rhoVaporw*L)) * pow(0.057*rhoLiquidw*Cpw/3.141592,0.5));
...
//                   const scalarField pwUnal(pw.patchInternalField());

//                   scalarField DUnal (max(1e-6,(0.0000242 * pow(pwUnal,0.709)*A/(bUnal*pow(phiUnal,0.5)))));
...
////                                               << kw[patchi] << "\t"
////                                               << TsubUnal[patchi] << "\t"
////                                               << TsupUnal[patchi] <<   endl;
////                                      }
////                       Info << "El diametro Unal - Max: " << max(DUnal) << " - Min: " <<  min(DUnal) << endl;
////                       Info << "El diametro Tolubs - Max: " << max(dDep_) << " - Min: " <<  min(dDep_) << endl;

//                   dDep_ = 1.0*DUnal;

////                   Info << "El diametro Tolubs Modif Unal - Max: " << max(dDep_) << " - Min: " <<  min(dDep_) << endl;
                ...
                //Info << "Reporto calcAlphat(*this)*(1 - fLiquid): " << alphatConv_ << endl;
            ...
            // ------------------------------------------------------------- //
            // Ojo esto lo agrego para reportar los flujos de calor, el diametro de burbuja,
            // la densidad de nucleacion y la frecuencia de desprendimiento
            const scalarField qee
                    (
                       fLiquid*qe() //fLiquid*(pi/6) * (dDep_)*(dDep_)*(dDep_) * fDep * N * rhoVaporw * L
                     );
...
//            Info << "Reporto los flujos de calor en liqudo"<<endl;
//            //Info << "z[m]" <<"\t" << "Qtotal" << " \t \t" << "Qquenching" << "\t" << "Qconvection" << "\t" << "Qevapor" << endl;
//            Info << "z[m]" <<"\t" << "Qtotal" << " \t \t" << "Qquenching" << "\t"
//                 << "Qconvection" << "\t" << "Qevapor" << "\t" << "fLiquid" << "\t" << "Fraccion alphaLiq" << "\t"  << "Twall" << endl;
//            forAll(qEff,patchi)
//            {
//             Info << patch().Cf()[patchi][2] << "\t"
//                  << qEff[patchi] << "\t"
...
//                    Info<< "Iteracion bucle Newton: " << i <<  " - Error bucle: " << maxErr << endl;
