...
// Ojo esta es una funcion privada de la clase Momentum TransferPhaseSystem que agrega
// los termino de fuentes de transferencia de mas
...
addMassTransferMomentumTransfer(phaseSystem::momentumTransferTable& eqns) const
{
    ...
    )
    {
        ...
        // Tenga en cuenta que la fase UEqn contiene un término de error de continuidad,
        // que implícitamente agrega un término de transferencia de
        // masa de fvm :: Sp (dmdt, U). Estas adiciones no incluyen este término.
...
//Ojoooooo!!! Aca esta como carga los terminos interfaciales mediante una tabla de punteros.!!!
...
Foam::MomentumTransferPhaseSystem<BasePhaseSystem>::momentumTransfer()
{
    ...
    // Add the virtual mass force - Agrega las fuerzas de masa virtual
    ...
    // Agregua el término fuente debido a la transferencia de masa
...
// Aca crea la tabla de puntero para los termino interfaciles pero de superficie
